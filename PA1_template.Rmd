...
```
The mean total number of steps is `r meanTotalSteps` and the median total number of steps is  `r medianTotalSteps`.
...
The time interval with maximum average number of steps is `r intervalMax`.
...
```
Only the steps variable contains missing values. There are `r missingValues` row with missing values in the dataset.
...
The new mean total number of steps is `r meanTotalSteps1` and the new median total number of steps is  `r medianTotalSteps1`, which are different from the estimates from the first part. As a result of imputing missing data, the mean and median of the estimates of the total daily number of steps became equal.
